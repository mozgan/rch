...
TEST(OgkOracle, RobustbaseJsonlFixturesMatchWithinPinnedTolerances) {
    simdjson::padded_string tolerance_json;
    ASSERT_EQ(simdjson::padded_string::load(oracle_path("ogk_tolerances.json"))
                  .get(tolerance_json),
              simdjson::SUCCESS);
    ...
    std::ifstream input{oracle_path("ogk_oracles.jsonl")};
    ...
    while (std::getline(input, line)) {
        ...
        const auto actual = ogk(std::span<const Vec3<double>>{points});
